...
continuous estimate of the relative state between the UAV and the target
...
more difficult for typical visual odometry algorithms, resulting in decreased
...
approaches
the landing target to avoid obstacles and ensure a gentle landing.
...
target vehicle.
Simulation and hardware tests of the proposed algorithm demonstrate accurate and consistent estimates
even when the fiducial marker is undetected
for long periods of time.
...
presented to generate dynamically feasible trajectories for UAVs landing on a moving
vehicle. One such method uses real-time model-predictive control techniques to
